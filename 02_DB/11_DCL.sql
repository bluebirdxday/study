 ...
 * - 사용자(USER)에 DB 또는 DB 객체에 대한 접근 권한을 
...
ALTER USER jhj_sample DEFAULT TABLESPACE SYSTEM
...
-- 1. JHJ 계정의 EMPLOYEE 테이블 조회
SELECT * FROM BLU.EMPLOYEE;
...
-- 3. 다시 JHJ 계정의 EMPLOYEE 테이블 조회
SELECT * FROM BLU.EMPLOYEE;


-- 4. 선동일 삭제
DELETE FROM BLU.EMPLOYEE
WHERE EMP_ID = 200;
-- ORA-01031: 권한이 불충분합니다
...
--> sample 계정에 시퀀스, 프로시저, 클러스터, 인덱스타입, 연산자
--  			타입, 트리거, 테이블 생성 권한 부여
...
GRANT CREATE SEQUENCE,
	  ...
	  CREATE TYPE,
	  CREATE TRIGGER,
	  CREATE TABLE TO jhj_sample;
-- 권한을 주고 싶으면 원래 이렇게 써야됨
	 
--> ROLE을 이용해서 간단히 부여 / 회수
GRANT RESOURCE TO jhj_sample;	
REVOKE RESOURCE FROM jhj_sample;



-- 사용자에게 부여된 시스템 권한 조회
-- SELECT * FROM DBA_SYS_PRIVS
-- WHERE GRANTEE = 'JHJ_SAMPLE'; 
...
-- GRANT CONNECT, RESOURCE, CREATE VIEW TO 계정명; (기본 권한들)(역할 2개와 권한 하나)
GRANT CONNECT, RESOURCE, CREATE VIEW TO jhj_sample;
