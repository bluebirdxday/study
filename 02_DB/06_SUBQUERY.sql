...
WHERE EXISTS (SELECT EMP_ID FROM EMPLOYEE SUB
             ...
             -- 서브쿼리의 MANAGER_ID가 아니라
			 -- 메인쿼리의 MANAGER_ID와 비교한다는 걸 명시해줌
...
WHERE HIRE_DATE 
     = (
         ...
         WHERE (ENT_YN = 'N' AND MAIN.DEPT_CODE = SUB.DEPT_CODE) -- NULL X인 경우
         OR 
         (ENT_YN = 'N' AND MAIN.DEPT_CODE IS NULL AND SUB.DEPT_CODE IS NULL)
         -- NULL인 경우
     )
ORDER BY HIRE_DATE;


-- 위와 동일하지만 코드는 좀 더 간결한 것 
SELECT EMP_ID, EMP_NAME, NVL(DEPT_TITLE, '소속없음') 부서명, JOB_NAME, HIRE_DATE
FROM EMPLOYEE MAIN
LEFT JOIN DEPARTMENT ON(DEPT_ID=DEPT_CODE)
JOIN JOB USING(JOB_CODE)
WHERE HIRE_DATE 
     = (
         SELECT MIN(HIRE_DATE) FROM EMPLOYEE SUB
         WHERE (ENT_YN = 'N' AND NVL(MAIN.DEPT_CODE, '1') = NVL(SUB.DEPT_CODE, '1'))
     )
ORDER BY HIRE_DATE;
...
-- 인라인뷰를 활용한 TOP-N분석  (***많이 쓰이니까 알아두기)
...
-- 해결 방법 : 인라인뷰(FROM절에 작성되는 서브쿼리)를 이용
...
--  가상의 테이블(VIEW)로 이용 (행 순서, 컬럼 그대로 이용)

-- 해결 2) 해결 1의 조회 결과를 FROM절에 작성하여 상위 5행만 조회
SELECT ROWNUM, EMP_NAME, SALARY 
FROM (SELECT EMP_NAME, SALARY 
	  FROM EMPLOYEE 
	  ORDER BY SALARY DESC) --> 메인쿼리에 포함된 VIEW 생성 구문 : 인라인뷰
WHERE ROWNUM <= 5;
...
-- 이러한 특수 함수들은 SELECT 절에만 사용 가능 -> 인라인절 이용하기
...
-- 1. ROWNUM 2. RANK 3. DENSE_RANK OVER 순으로 많이 쓰임
