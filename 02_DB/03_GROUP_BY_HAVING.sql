...
-- [937] [42000]: ORA-00937: 단일 그룹의 그룹 함수가 아닙니다

SELECT DEPT_CODE, SUM(SALARY) -- 3) 그룹 별 SALARY 합계를 조회
...
ORDER BY JOB_CODE;
-- COUNT(*) : 직급코드별 사원수
-- COUNT(BONUS) : NULL을 제외하고 카운트를 하기 때문에 직급코드 별 보너스 사원 수 조회 가능
...
-- WHERE + GROUP BY
--> WHERE절이 GROUP BY절 보다 우선순위가 높다(빠르다)
		

-- EMPLOYEE 테이블에서 부서코드가 'D4', 'D6'인 부서의 평균 급여 조회
SELECT DEPT_CODE, AVG(SALARY) -- 4) DEPT_CODE와 급여 평균 조회
FROM EMPLOYEE -- 1) EMPLOYEE 테이블에서
WHERE DEPT_CODE IN ('D5', 'D6') -- 2) 부서코드가 'D4', 'D6'인 행만 추려서
...
/*2*/HAVING AVG(SALARY) >= 3000000 -- GROUB BY에서 묶인 그룹에 조건을 대입
/*4*/ORDER BY 1;

-- 부서별 급여가 300백만 이상인 사원들의 평균급여를 조회
SELECT DEPT_CODE, AVG(SALARY) 
FROM EMPLOYEE
...
-- 부서코드, 평균 급여(소수점 내림)를 부서코드 내림차순 초회
...
-- 집계 함수
-- GROUP BY절에 작성하여 그룹별로 산출한 결과를 집계하는 함수
-- ROLLUP, CUBE가 있음

-- ROLLUP : 그룹별로 중간 집계와 전체 합계를 처리하는 함수

-- EMPLOYEE 테이블에서 각 부서에 소속된 직급별 급여합, 부서별 급여합, 전체직원의 급여합 조회
SELECT DEPT_CODE, JOB_CODE, SUM(SALARY) 
FROM EMPLOYEE
...
-- 두개 이상의 SELECT 결과(RESULT SET)을 이용해 하나의 결과를 조회하는 연산
...
-- INTERSECT 확인
-- 부서코드가 'D5'이면서 급여가 300만 초과인 사원 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY 
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
INTERSECT
