...
-- 단순한 상황일 때 주로 사용, SELECT문 길어지면 복잡해짐
...
-- 복잡한 SELECT에선 주로 이 별칭을 지정하는 방법 사용 
...
-- 두 테이블의 지정하는 컬럼값이 일치하지 않는 행도 조인에 포함을 시킴 (> NULL도 보이게 만든다는 의미)
...
-- 같은 테이블을 조인.
-- 자기 자신과 조인을 맺음
--> 하나의 테이블을 가지고 생각하지 말고 같은 테이블이 두개가 있다고 생각하면 쉬움

-- EMPLOYEE 테이블에서 사번, 이름, 사수사번, 사수이름 조회
-- ANSI 표준
SELECT EMP.EMP_ID, EMP.EMP_NAME, 
	NVL(EMP.MANAGER_ID, '없음') , NVL(MGR.EMP_NAME, '없음')    -- NULL값 => 없음 이라고 표기하기
FROM EMPLOYEE EMP
LEFT JOIN EMPLOYEE MGR ON(MGR.EMP_ID = EMP.MANAGER_ID);
...
-- * JOIN 순서 중요!!! *
--> 앞선 두 테이블(EMPLOYEE, DEPARTMENT)의 조인 후 조인된 결과에 LOCATION 테이블을 조인
...
WHERE SAL_LEVEL AND SALARY > MIN_SAL;
...
-- SELF JOIN 사용
-- 동료 == 같은 부서 == 같은 부서 코드
SELECT EMP.EMP_NAME 사원명, EMP.DEPT_CODE, COW.EMP_NAME 동료이름
FROM EMPLOYEE EMP
JOIN EMPLOYEE COW ON(COW.DEPT_CODE = EMP.DEPT_CODE)
WHERE EMP.EMP_ID != COW.EMP_ID
ORDER BY 1;
