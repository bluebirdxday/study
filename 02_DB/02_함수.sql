...
-- 그룹 함수 : N개의 행의 값을 하나의 그룹으로 묶어 그룹 수만큼의 결과를 반환
...
SELECT 'AABAACAABBA', INSTR('AABAACAABBA', 'B') FROM DUAL; --3

-- 문자열을 5번째부터 검색하여 처음 찾는 B의 위치
SELECT 'AABAACAABBA', INSTR('AABAACAABBA', 'B', 5) FROM DUAL; --9
                           --123456789
-- 문자열을 5번째부터 검색하여 두번째로 찾는 B의 위치
SELECT 'AABAACAABBA', INSTR('AABAACAABBA', 'B', 5, 2) FROM DUAL; --10
                           --1234567890
...
-- 주어진 문자열의 앞 or 뒤 or 양쪽에 존재하는 지정된 문자열을 제거
...
ROUND(123.456, 1), -- 소수점 첫째 자리까지 표현, 둘째 자리에서 반올림 -> 123.4
ROUND(123.456, 2), -- 소수점 둘째 자리까지 표현, 셋째 자리에서 반올림 -> 123.46
ROUND(123.456, 0), -- 정수
ROUND(123.456, -1), -- 소수점 -1번째 자리까지 표현, 정수 1의 자리에서 반올림 -> 123
ROUND(123.456, -2)  -- 소수점 -2번째 자리까지 표현, 정수 10의 자리에서 반올림 -> 120
FROM DUAL;
...
SELECT FLOOR(123.5), TRUNC(123.5)  --> FLOOR 내리는 것, TRUNC 버리는 것 -> 둘 다 123
FROM DUAL;

SELECT FLOOR(-123.5), TRUNC(-123.5)  --> -124 / -123
FROM DUAL;


SELECT EMP_NAME, TRUNC(SALARY, -6) || '원 이상' 급여  --> 10만 자리까지 잘라서 100만 단위만 남음 
FROM EMPLOYEE;
...
-- 2023-03-07 12:22:43.000, 2023-03-07 12:22:43.028 +0900
 ...
 * 2) 0 : 숫자 할 칸을 의미, 오른쪽 정렬, 빈칸에 0을 추가
...
SELECT 1000000,
	TO_CHAR(1000000, 'L9999,999'),  --         ￦1000000
	TO_CHAR(1000000, '$9999999'),   -- $1000000
	TO_CHAR(1000000, '$9,999,999')  -- $1,000,000
FROM DUAL;


-- EMPLOYEE 테이블에서 사번, 이름, 연봉 조회(원화표시100,000,000 형식으로 조회)
...
SELECT SYSDATE, TO_CHAR(SYSDATE, 'AM HH:MI:SS') FROM DUAL;  -- 오후 02:25:58

SELECT SYSDATE, TO_CHAR(SYSDATE, 'YY-MM-DD HH:MI:SS') FROM DUAL; -- 23-03-07 02:26:58
...
SELECT EMP_NAME, TO_CHAR(HIRE_DATE, 'RRRR"년" MM"월" DD"일" (DY)')  -- 소괄호는 쌍따옴표 써줄 필요 x
FROM EMPLOYEE;
...
SELECT TO_DATE(2023년 03월 07일) FROM DUAL; 
...
/* 날짜 패턴 Y, R의 차이점 */
-- 년도를 짧게 해석하는 경우 
-- 50미만 : Y, R 둘 다 앞부분에 현재 세기(21C == 2000년대)를 적용
-- 50이상 : Y == 현재 세기(2000년대)
--		   R == 이전 세기(1980년대)
...
/* DB에서 NULL과 연간하는 경우 모든 결과는 NULL*/
SELECT EMP_NAME, SALARY, NVL(BONUS, 0), SALARY * NVL(BONUS, 0)
FROM EMPLOYEE;            -- 둘의 타입은 동일해야함 NVL(숫자형, 문자열) -> 오류남
