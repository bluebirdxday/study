...
VALUES(900, '장채현', '901123-2345678', 'jang_ch@kh.or.kr', '01012341234','D1', 'J7', 'S3', 
...
ROLLBACK; 
-- 마지막 COMMIT 시점으로 돌아가라

COMMIT; 
-- 수행된 DML의 결과를 DB에 반영(저장)
...
-- SELECT 결과의 행들을 EMP_01에 삽입하라

SELECT * FROM EMP_01;

-- 서브 쿼리를 이용해서 다량의 정보 삽입 가능
...
-- DEPARTMENT2 테이블에서 BONUS를 받지 않는 사원의 
-- BONUS를 0.1로 변경
SELECT * FROM EMPLOYEE2;
...
SELECT * FROM DEPARTMENT2;
--> 조건을 설정하지 않으면 지정된 테이블의 모든 행이 수정됨
--> WHERE절 작성을 잘 하자
...
SET (SALARY, BONUS) = (SELECT SALARY, BONUS
                       FROM EMPLOYEE2
                       WHERE EMP_NAME = '유재식')  -- SELECT~문 -> 다중열 서브쿼리
