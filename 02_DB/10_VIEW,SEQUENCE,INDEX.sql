 ...
 * 
 * - 논리적 가상 테이블 
 *    -> 테이블 모양을 하고는 있지만 실제로 값을 저장하고 있지 않음
 * 
 ...
 *
 * 	  데이터베이스 이관 작업, 종류를 바꿀 때 사용 -> 대부분 사용할 일 없다는 의미
 * 
...
GRANT CREATE VIEW TO BLU;

--> 다시 기존 계정으로 접속해서 VIEW 생성 구문 수행
...
-- INSERT 확인
SELECT * FROM V_DCOPY2;
--> VIEW는 값을 저장하지 않는다고 했는데 저장된 것처럼 보인다
--> 실제 VIEW에 저장된 것이 아니라 VIEW를 생성할 때 사용한 서브쿼리의 테이블(DEPT_COPY2)에
--  값이 삽입되어 있다
...
-- D0	NULL	L3
--> 원본 테이블에 삽입됨을 확인
---> INSERT 구문에 포함되지 않은 DEPT_TITLE 컬럼은 NULL이 삽입되어 있다 (중요)
...
--> 뜻 : 원본 테이블 DEPT_ID, LOCATION_ID 컬럼에 'D0', 'L3' 삽입
--> VIEW의 이름을 적었지만 원본에 삽입하려고 함
--> DEPT_TITLE 컬럼에는 NULL 삽입
--> 그런데, DEPT_TITLE에는 NOT NULL 제약조건이 설정되어 NULL 값이 삽입될 수 없음

-- ORA-01400: NULL을 ("BLU"."DEPT_COPY2"."DEPT_TITLE") 안에 삽입할 수 없습니다
...
-- 읽기 전용 X로 VIEW 변경
CREATE OR REPLACE VIEW V_DCOPY2 -- 새로 만들거나 기존에 있으면 변경하겠다
 ...
 * 
 * *** SEQUENCE 왜 사용할까? ***
 * PRIMARY KEY(기본키) : 테이블 내 각 행을 구별하는 식별자 역할을 함
 ...
 *  [작성법]
  ...
  --> EX) START : 1, INCREMENT : 1, MAXVALUE : 3, MINVALUE : -3이라면
          1,2,3 돌고 -3, -2, -1, 0 순회함
...
SELECT * FROM (SELECT TEST_NO FROM TB_TEST
...
-- 시퀀스는 START WITH를 변경하고 싶을 때 삭제 후 다시 생성해야 함


--------------------------------------------------------------------

/* INDEX(색인)
 * - SQL 구문 중 SELECT 처리 속도를 향상시키기 위해 컬럼에 대하여 생성하는 객체
 * 
 * - 인덱스 내부 구조는 B* 트리 형식으로 되어있음
 * 
 * ** INDEX의 장점 **
 * - 이진 트리 형식으로 구성되어 자동 정렬 및 검색 속도 향상
 * - 조회 시 테이블의 전체 내용을 확인하며 조회하는 것이 아닌, 
 *   인덱스가 지정된 컬럼만을 이용해서 조회하기 때문에 시스템의 부하가 낮아짐
 * 
 * ** 인덱스의 단점 **
 * - 데이터 변경(INSERT, UPDATE, DELETE) 작업 시 이진 트리 구조의 변형이 일어남
 ...
 * 
 * ** 인덱스가 자동 생성되는 경우 **
 * -> PK 또는 UNIQUE 제약조건이 설정된 컬럼에 대해 UNIQUE INDEX가 자동 생성됨
 * */
...
CREATE INDEX INX_TEST_ID
ON TB_INDEX_TEST(TEST_ID);

-- 인덱스 생성 후 조회
SELECT * FROM TB_INDEX_TEST
WHERE TEST_ID = 'TEST500000'; -- 23/1/1/1 ms



-- 인덱스 삭제
DROP INDEX INX_TEST_ID;

SELECT * FROM TB_INDEX_TEST
WHERE TEST_ID = 'TEST500000';  -- 65/32/40 ms
