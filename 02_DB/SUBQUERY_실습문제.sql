...
WHERE HIRE_DATE = (SELECT MIN(HIRE_DATE)
				   FROM EMPLOYEE SUB
				   WHERE NVL(SUB.DEPT_CODE, '1') = NVL(MAIN.DEPT_CODE, '1')
				   AND ENT_YN = 'N')
ORDER BY HIRE_DATE;
...
FROM EMPLOYEE MAIN
JOIN JOB USING(JOB_CODE)
WHERE (JOB_CODE, TRUNC(MONTHS_BETWEEN(SYSDATE, TO_DATE(SUBSTR(EMP_NO, 1, 6)))/12))
IN (SELECT JOB_CODE, MIN(TRUNC(MONTHS_BETWEEN(SYSDATE, TO_DATE(SUBSTR(EMP_NO, 1, 6)))/12))
   FROM EMPLOYEE SUB
   GROUP BY JOB_CODE)
 ORDER BY 나이 DESC;
...
WHERE TRUNC(MONTHS_BETWEEN(SYSDATE, TO_DATE(SUBSTR(EMP_NO, 1, 6)))/12)
= (SELECT MIN(TRUNC(MONTHS_BETWEEN(SYSDATE, TO_DATE(SUBSTR(EMP_NO, 1, 6)))/12))
   FROM EMPLOYEE SUB
