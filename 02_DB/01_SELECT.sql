...
-- 컬럼 값 : 행과 열이 교차되는 테이블의 한 칸(== 한 셀)에 작성된 값
...
SELECT EMP_ID, EMP_NAME, SALARY, SALARY + 100000 FROM EMPLOYEE;
...
-- EMPLOYEE 테이블에서 이름, 입사일, 오늘까지 근무할 날짜 조회
...
--EMPLOYEE 테이브에서 부서코드가 'D6'이고 급여가 200만 이상인 사원의 이름, 급여, 부서코드 조회
...
-- EMPLOYEE 테이블에서 급여가 300만 이상 500만 이하인 직원의 모든 컬럼 조회
SELECT *
FROM EMPLOYEE
...
-- NOT BETWEEN A OR B : 컬럼 값이 A 이상 B 이하가 아닌 경우
-- EMPLOYEE 테이블에서 급여가 300만 이상 500만 이하가 아닌 직원의 모든 컬럼 조회
-- == 300만 미만 500만 초과
SELECT *
FROM EMPLOYEE
...
-- EMPLOYEE 테이블에서
-- 부서 코드가 'D5' 또는 'D6' 또는 'D9'인 사원의 사번, 이름, 부서코드 조회
...
-- EMPLOYEE 테이블에서
-- 부서 코드가 'D5' 또는 'D6' 또는 'D9'이 아닌 사원의 사번, 이름, 부서코드 조회
...
-- EMPLOYEE 테이블에서
-- 전화번호가 '010'으로 시작하는 사원의 사번, 이름, 전화번호 조회
...
-- EMPLOYEE 테이블에서
-- 전화번호가 '010'으로 시작하지 않는 사원의 사번, 이름, 전화번호 조회 
-- 전화번호가 NULL인 사원도 함께 조회
SELECT EMP_ID, EMP_NAME, PHONE
FROM EMPLOYEE
WHERE PHONE NOT LIKE '010%'
OR PHONE IS NULL;

-- EMPLOYEE 테이블에서 이메일에 @ 앞글자가 5글자인 사원의 
-- 사번, 이름, 이메일 조회
...
-- EMPLOYEE 테이블에서 이메일에 _ 앞글자가 3글자인 사원의 
-- 사번, 이름, 이메일 조회
SELECT EMP_ID, EMP_NAME, EMAIL
FROM EMPLOYEE
...
-- WHERE HIRE_DATE >= '1990-01-01' 이것도 가능
 ...
 * 1: FROM 테이블명
 * 2: WHERE 조건식
 * 4: ORDER BY 컬럼명 | 별칭 | 컬럼 순서 [오름 ASC/내림차순 DESC]
 *          [NULLS FIRST | LAST] 
 * */
...
FROM EMPLOYEE
ORDER BY SALARY ASC;
     --> 오름차순(ASC)가 기본값 ORDER BY SALARY 와 동일한 결과 

-- 급여 내림차순으로 조회
SELECT EMP_NAME, SALARY
FROM EMPLOYEE
ORDER BY SALARY DESC;
	 --> 내림차순을 원할 경우 DESC를 작성
...
/*1*/FROM EMPLOYEE
/*2*/WHERE 연봉 >= 50000000; 
-- 오류 발생(SELECT절 해석이 안 된 상태에서 별칭을 WHERE절에 작성)
...
ORDER BY PHONE /*NULLS LAST*/ NULLS FIRST;
             -- 오름차순 기본값 : NULLS LAST (맨 뒤에 NULL 배치)


-- 전화번호 내림차순 조회
SELECT EMP_NAME, PHONE 
FROM EMPLOYEE
ORDER BY PHONE DESC /*NULLS FIRST*/ NULLS FIRST;
...
-- 부서 코드별 급여 내림차순 (부서 코드는 내림차순)
SELECT EMP_NAME, DEPT_CODE, SALARY 
FROM EMPLOYEE
ORDER BY DEPT_CODE DESC NULLS FIRST, SALARY DESC;
