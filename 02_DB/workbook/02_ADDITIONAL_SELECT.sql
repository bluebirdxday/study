...
CEIL(MONTHS_BETWEEN(TO_DATE(SUBSTR(PROFESSOR_SSN, 1, 6), 'YYMMDD'), SYSDATE)/12) 나이
FROM TB_PROFESSOR
...
FROM TB_STUDENT
WHERE 
MONTHS_BETWEEN(TO_DATE(SUBSTR(STUDENT_SSN, 1, 6),'YYMMDD'), ENTRANCE_DATE)/12 > 20;
...
-- 13. ------------------------------------------------- ~~~~!!!
SELECT DEPARTMENT_NO "학과코드명", NVL(COUNT(ABSENCE_YN), 0) "휴학생 수"
FROM TB_STUDENT
WHERE ABSENCE_YN = 'Y'
GROUP BY DEPARTMENT_NO
