...
// -1 == 초기값, 0 == 프로필 삭제(x버튼), 1 == 새이미지 업로드

let originalImage;
// 초기 프로필 이미지 파일의 경로 저장

if(imageInput!= null){      //화면에 imageInput이 있을 경우
    
    // 프로필 이미지가 출력되는 img 태그에 src 속성을 저장
    ...
    imageInput.addEventListener("change", e=>{
        ...
        if (file.size>maxSize){        // 선택된 파일의 크기가 최대 크기를 초과하는 경우
            alert("2MB 이하의 이미지를 선택해주세요");
            imageInput.value = "";
            // input type="file" 태그에 대입할 수 있는 value는 " "(빈칸) 뿐이다!
            deleteCheck = -1;      // 취소 == 파일 없음 == 초기 상태

            // 용량 초과 시 기존 프로필 이미지로 변경
            profileImage.setAttribute("src", originalImage);
            return;
        }

        // JS에서 파일을 읽는 객체
        // - 파일을 읽고 클라이언트 컬퓨터에 파일을 저장할 수 있음
    ...
    document.getElementById("profileFrm").addEventListener("submit", e =>{
        ...
        // 제출 할 수 있는 경우를 따져보기  (제출할 수 없는 경우의 수가 더 많아서)
        ...
        if(flag){       // flag == true -> 제출하면 안되는 경우
            e.preventDefault();       // form 기본 이벤트 제거(제출 안됨)
