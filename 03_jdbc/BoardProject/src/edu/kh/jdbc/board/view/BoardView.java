...
import edu.kh.jdbc.board.model.view.CommentView;
...
public class BoardView {
	...
	private void updateBoard(int boardNo) {
		...
		StringBuffer buffer = new StringBuffer();  // StringBuffer(가변성)
		...
		try {
			...
			// 게시글 수정 서비스 호출
			int result = boardService.updateBoard(boardTitle, 
					buffer.toString(), boardNo);
	...
	private void insertBoard() {
		...
		try {
			...
			int result = boardService.insertBoard(boardTitle, 
					buffer.toString(), Session.loginMember.getMemberNo());
			...
			if(result>0) {
				...
				Board board = boardService.selectBoard(result, 
