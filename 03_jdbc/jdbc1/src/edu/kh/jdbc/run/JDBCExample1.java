...
public class JDBCExample1 {
	...
	public static void main(String args[]) {
		 ...
		 *     -> (ojdbc11.jar 의 OracleDriver.class)
		 * 
		 * */
		...
		Statement stmt = null;
		// sql을 DB에 전달해 수행하고 결과(ResultSet 또는 DML 성공 행의 개수)를 반환 받는 데 사용되는 객체
		...
		try {
			...
			// -> 메모리에 객체가 생성되고 Connection을 이용한 DB 연결 시 이를 자동으로 참조한다
			
			
			// ClassNotFoundException 발생 가능성이 있음
			// -> 오타 또는 라이브러리 추가를 안했을 시 발생
			...
			String ip = "localhost";
			// localhost = 127.0.0.1 (loop back ip == 자기 컴퓨터)
			...
			String pw = "Nj09121007!";  // 비밀번호
			
			
			
			// DriverManager 객체
			// - 메모리에 로드된 JDBC드라이버 객체를 이용해 DB에 접근하고
			//   작성된 정보를 이용해 DB와 연결 가능한 Connection 객체를 만들어 얻어오는 역할
...
//			System.out.println(conn);
			...
			// executeQuery() : SELECT문을 수행하는 메서드. 결과로 ResultSet 반환
